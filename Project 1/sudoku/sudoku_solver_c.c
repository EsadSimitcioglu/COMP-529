...
#pragma omp single
            {
                ...
                {
                    if (canBeFilled(matrix, row, col, num, box_sz, grid_sz) && !solutionFound)
                    {
#pragma omp task firstprivate(row, col, num, depth)
                        {
                            depth = depth + 1;
                            matrix[row][col] = num;
...
int canBeFilled(int matrix[MAX_SIZE][MAX_SIZE], int row, int col, int num, int box_size, int grid_sz)
{
    ...
    int firstCond = !existInRow(matrix, row, num, box_size);
    int secondCond = !existInColumn(matrix, col, num, box_size);
    int thirdCond = !existInGrid(matrix, row - row % grid_sz, col - col % grid_sz, num, grid_sz);

    return firstCond && secondCond && thirdCond && matrix[row][col] == EMPTY;
}
