...
void divideData(double **src, double **dst, int x, int y, int n, int m, int rank)
{
  double **s_dst;
  double **s_src;
  s_src = alloc2D(m, n);
...
int main(int argc, char **argv)
{
  ...
  if (no_comm == 0)
  {
    divideData(E, my_E, x_size, y_size, n, m, rank);
    divideData(E_prev, my_E_prev, x_size, y_size, n, m, rank);
    divideData(R, my_R, x_size, y_size, n, m, rank);
  }
