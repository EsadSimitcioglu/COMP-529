    ...
    float3 *meshNormals)
{
    ...
    for (int ix = index * (NumX / (threadNum / blockNum)); ix <= index * (NumX / (threadNum / blockNum)) + (NumX / (threadNum / blockNum)); ix++)
    {
        for (int iy = 0; iy < NumY; ++iy)
        {
            for (int iz = 0; iz < NumZ; ++iz)
...
__global__ void MarchCubeCUDAMultiframe(
    ...
    float3 *meshVertices,
    float3 *meshNormals,
    float3 *meshVertices_buffer,
    float3 *meshNormals_buffer)
{
    int NumX = static_cast<int>(ceil(domainP->size.x / cubeSizeP->x));
    int NumY = static_cast<int>(ceil(domainP->size.y / cubeSizeP->y));
    int NumZ = static_cast<int>(ceil(domainP->size.z / cubeSizeP->z));
    ...
    {
        float twist = 1.0 / float(frameNum) * maxTwist * frame;
        ...
        for (int ix = index * (NumX / (threadNum / blockNum)); ix < index * (NumX / (threadNum / blockNum)) + (NumX / (threadNum / blockNum)); ix++)
        {
            ...
            for (int iy = 0; iy < NumY; ++iy)
            {
                for (int iz = 0; iz < NumZ; ++iz)
                {
                    ...
                    int totalNumVertices = NumX * NumY * NumZ * 16;

                    int globalIdx = (NumZ * NumY * ix + NumZ * iy + iz) * 16;

                    for (int i = 0; i < 16; ++i)
                    {
                        int idx = globalIdx + i;

                        if (idx < totalNumVertices)
                        {
                            meshVertices_buffer[idx + totalNumVertices * frame] = meshVertices[idx];
                            meshNormals_buffer[idx + totalNumVertices * frame] = meshNormals[idx];

                            meshVertices[idx] = make_float3(0, 0, 0);
                            meshNormals[idx] = make_float3(0, 0, 0);
                        }
                    }
                }
            }
...
int main(int argc, char *argv[])
{
    ...
    // float3 *intersect;
    // checkCudaErrors(cudaMalloc((void**)&intersect, 12 * sizeof(float3)));
    // cudaMemset(intersect, 0, 12 * sizeof(float3));
    ...
    //                        PART 1&2 RUN:                          //
    ///////////////////////////////////////////////////////////////////
    if (part== -2 || part == -1 || part == 1 || part == 2)
    {
        for (frame = 0; frame < frameNum; frame++)
        {
            start = high_resolution_clock::now();
            ...
            //                   Launch the kernel                   //
            ///////////////////////////////////////////////////////////
            // dim3 dimNumBlocks(numBlocks, numBlocks, numBlocks);
            // dim3 dimNumThreads(numThreads, numThreads, numThreads);

            MarchCubeCUDA<<<numBlocks, numThreads / numBlocks>>>(numBlocks, numThreads, domain_d, cubeSize_d, twist, 0, meshVertices_d, meshNormals_d);
            ...
            checkCudaErrors(cudaMemcpy(meshVertices_h, meshVertices_d, frameSize * sizeof(float3), cudaMemcpyDeviceToHost));
            checkCudaErrors(cudaMemcpy(meshNormals_h, meshNormals_d, frameSize * sizeof(float3), cudaMemcpyDeviceToHost));
            ...
            checkCudaErrors(cudaMemset(meshNormals_d, 0, frameNum * frameSize * sizeof(float3)));
            checkCudaErrors(cudaMemset(meshVertices_d, 0, frameNum * frameSize * sizeof(float3)));

            checkCudaErrors(cudaMemset(meshNormals_h, 0, frameNum * frameSize * sizeof(float3)));
            checkCudaErrors(cudaMemset(meshVertices_h, 0, frameNum * frameSize * sizeof(float3)));
        }
    ...
    int totalNumVertices = NumX * NumY * NumZ * 16;
    int totalVerticesAllFrames = totalNumVertices * frameNum;

    float3 *meshVertices_buffer;
    checkCudaErrors(cudaMallocHost(&meshVertices_buffer, totalVerticesAllFrames * sizeof(float3)));
    float3 *meshNormals_buffer;
    checkCudaErrors(cudaMallocHost(&meshNormals_buffer, totalVerticesAllFrames * sizeof(float3)));

    checkCudaErrors(cudaMallocHost(&meshVertices_h, totalVerticesAllFrames * sizeof(float3)));
    checkCudaErrors(cudaMallocHost(&meshNormals_h, totalVerticesAllFrames * sizeof(float3)));
    ...
    if (part == -1 || part == 3)
    {
        ...
        //                     Launch the kernel                     //
        ///////////////////////////////////////////////////////////////
        MarchCubeCUDAMultiframe<<<numBlocks, numThreads / numBlocks>>>(numBlocks, numThreads, domain_d, cubeSize_d, frameNum, maxTwist, 0, meshVertices_d, meshNormals_d, meshVertices_buffer, meshNormals_buffer);
        checkCudaErrors(cudaDeviceSynchronize());
        end = high_resolution_clock::now();
        kernelTime = (duration<double>(end - start)).count();
        ...
        checkCudaErrors(cudaMemcpy(meshVertices_h, meshVertices_buffer, totalVerticesAllFrames * sizeof(float3), cudaMemcpyDeviceToHost));
        checkCudaErrors(cudaMemcpy(meshNormals_h, meshNormals_buffer, totalVerticesAllFrames * sizeof(float3), cudaMemcpyDeviceToHost));
    ...
    float3 *meshVertices_dd[2], *meshNormals_dd[2];

    checkCudaErrors(cudaMalloc(&meshVertices_dd[0], frameSize * sizeof(float3)));
    checkCudaErrors(cudaMalloc(&meshNormals_dd[0], frameSize * sizeof(float3)));
    checkCudaErrors(cudaMalloc(&meshVertices_dd[1], frameSize * sizeof(float3)));
    checkCudaErrors(cudaMalloc(&meshNormals_dd[1], frameSize * sizeof(float3)));
    ...
    if (part == -1 || part == 4)
    {
        ...
        for (frame = 0; frame < frameNum; frame++)
        {
            ...
            MarchCubeCUDA<<<numBlocks, numThreads / numBlocks, 0, computeStream>>>(numBlocks, numThreads, domain_d, cubeSize_d, twist, 0, meshVertices_dd[idx], meshNormals_dd[idx]);
            cudaEventRecord(computeDone, computeStream);

            if (frame > 0) {
                cudaStreamWaitEvent(transferStream, computeDone, 0);

                checkCudaErrors(cudaMemcpyAsync(meshVertices_h, meshVertices_dd[1 - idx], frameSize * sizeof(float3), cudaMemcpyDeviceToHost, transferStream));
                checkCudaErrors(cudaMemcpyAsync(meshNormals_h, meshNormals_dd[1 - idx], frameSize * sizeof(float3), cudaMemcpyDeviceToHost, transferStream));
            ...
            if (saveObj || correctTest) {
                ...
                checkCudaErrors(cudaMemcpy(meshVertices_h + offset, meshVertices_dd[idx], frameSize * sizeof(float3), cudaMemcpyDeviceToHost));
                checkCudaErrors(cudaMemcpy(meshNormals_h + offset, meshNormals_dd[idx], frameSize * sizeof(float3), cudaMemcpyDeviceToHost));
                ...
                checkCudaErrors(cudaMemset(meshVertices_dd[idx], 0, frameSize * sizeof(float3)));
                checkCudaErrors(cudaMemset(meshNormals_dd[idx], 0, frameSize * sizeof(float3)));
            }
        ...
        if (frameNum > 0) {
            int idx = (frameNum - 1) % 2;
            checkCudaErrors(cudaMemcpy(meshVertices_h + (frameNum - 1) * frameSize, meshVertices_dd[idx], frameSize * sizeof(float3), cudaMemcpyDeviceToHost));
            checkCudaErrors(cudaMemcpy(meshNormals_h + (frameNum - 1) * frameSize, meshNormals_dd[idx], frameSize * sizeof(float3), cudaMemcpyDeviceToHost));
        }
            end = high_resolution_clock::now();
            totalTime = (duration<double>(end - start)).count();
    ...
    //                         Free memory                           //
    ///////////////////////////////////////////////////////////////////
    checkCudaErrors(cudaFree(meshVertices_dd[0]));
    checkCudaErrors(cudaFree(meshNormals_dd[0]));
    checkCudaErrors(cudaFree(meshVertices_dd[1]));
    checkCudaErrors(cudaFree(meshNormals_dd[1]));
    
    
    checkCudaErrors(cudaFree(meshNormals_d));
    checkCudaErrors(cudaFree(meshVertices_d));

    checkCudaErrors(cudaStreamDestroy(computeStream));
    checkCudaErrors(cudaStreamDestroy(transferStream));

    checkCudaErrors(cudaEventDestroy(computeDone));
    checkCudaErrors(cudaEventDestroy(transferDone));
    cudaFree(meshVertices_h);
    cudaFree(meshNormals_h);
